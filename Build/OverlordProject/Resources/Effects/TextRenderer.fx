...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float4 col, float2 texCoord, int channel)
{
	...
	object.Position = mul(float4(pos,1), gTransform);
	object.Color = col;
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	position.y = vertex[0].Position.y;
    ...
    position.x = vertex[0].Position.x + vertex[0].CharSize.x;
    position.y = vertex[0].Position.y;
    CreateVertex(triStream, position, vertex[0].Color, texCoord, vertex[0].Channel);
	...
	position.x = vertex[0].Position.x;
    position.y = vertex[0].Position.y + vertex[0].CharSize.y;
    CreateVertex(triStream, position, vertex[0].Color, texCoord, vertex[0].Channel);
...
float4 MainPS(GS_DATA input) : SV_TARGET {
	...
	float sampled = gSpriteTexture.Sample(samPoint, input.TexCoord)[input.Channel];
